...
#Use Scrot to take a screen capture of the user-selected region, save the image to a file, and then use pngquant to optimize and overwrite that file.
#
#This script is bound to the Alt+Shift+d key combo in ~/.config/openbox/rc.xml .
#
#Scrot options are:
# -shole has 2 parts: the "s" makes it a user-selected screenshot, and "hole" creates a highlight rectangle and dims the rest of the screen. 
...
# $f pulls the full path of the image from Scrot
...
scrot -shole -f -q 99 -F $HOME/.tmp/tmp-scrot-output.png -e 'pngquant $f -s 10 -o $HOME/.tmp/tmp-scrot-output.png -f --strip'
